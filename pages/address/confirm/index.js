  ...
  getAddressList() {
    ...
    App.HttpService.getAddressList(params)
      .then(data => {
        console.log(data)
        if (data.code == 0) {
          address.items = address.items.concat(data.items)
          address.paginate = data.paginate
          address.params.page = data.paginate.next
          address.params.limit = data.paginate.perPage
