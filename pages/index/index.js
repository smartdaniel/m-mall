  ...
  getBanners() {
    ...
    this.banner.queryAsync({ is_show: !0 })
      .then(data => {
        console.log(data)
        if (data.code == 0) {
          data.items.forEach(n => n.path = App.renderImage(n.path));
          this.setData({
            images: data.items
          });
        }
    ...
    this.classify.queryAsync({
      ...
      .then(data => {
        ...
        if (data.code == 0) {
          this.setData({
            navList: data.items,
            'goods.params.type': data.items[activeIndex]._id
          })
  ...
  getList() {
    ...
    this.goods.queryAsync(params)
      .then(data => {
        console.log(data)
        if (data.code == 0) {
          data.items.forEach(n => n.thumb_url = App.renderImage(n.images[0] && n.images[0].path))
